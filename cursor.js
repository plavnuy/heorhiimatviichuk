...
// ==========================
// Pointer events (canvas → window, без изменения визуала)
// ==========================

// MOUSE MOVE — движение (dx / dy)
window.addEventListener('mousemove', e => {
  const p = pointers[0];

  p.moved = p.down;
  p.dx = (e.clientX - p.x) * 10.0;
  p.dy = (e.clientY - p.y) * 10.0;
  p.x = e.clientX;
  p.y = e.clientY;
});

// MOUSE MOVE — активация + цвет (как было на canvas)
